...
**배포 URL**: [https://codyssey-a3-topaz.vercel.app](https://codyssey-a3-topaz.vercel.app)
**GitHub**: [https://github.com/040611bluesky-creator/codyssey-A3](https://github.com/040611bluesky-creator/codyssey-A3)
...
집밥요정은 냉장고 속 재료를 입력하면 AI(Google Gemini)가 인원, 조리 시간, 입맛 취향에 맞는 집밥 레시피를 추천해주는 서비스입니다.

- **타겟 사용자**: 자취생, 요리 초보, "오늘 뭐 먹지" 고민하는 직장인
- **핵심 가치**: 냉장고 재료를 활용해 장을 보러 나가지 않고도, 지금 바로 만들 수 있는 메뉴를 빠르게 결정할 수 있도록 도와줌
...
| 400 | 재료 입력이 1000자를 초과 | "재료 입력은 1000자 이내로 입력해주세요" |
...
2. Vercel 프로젝트의 **Settings → Environment Variables**에 `GEMINI_API_KEY`를 등록합니다. (Production, Preview, Development 환경 모두 체크 권장)
...
현재 프로젝트는 HTML/CSS/JavaScript(Vanilla)와 Vercel Serverless Functions(Python) 구조로 구성되어 있습니다. 향후 프론트엔드 프레임워크로 전환하는 경우 다음 항목을 확인합니다.

1. **라우팅**
   - 기존 `#main`, `#recommend`, `#about` 섹션 이동 방식과 `/api/recommend` API 호출 경로가 새 구조에서도 정상적으로 연결되는지 확인합니다.
   - 정적 파일 경로와 API 경로가 충돌하지 않는지 확인합니다.

2. **빌드**
   - 프레임워크에 맞는 Build Command와 Output Directory를 확인합니다.
   - 로컬에서 빌드 및 실행하여 오류가 없는지 확인합니다.

3. **배포**
   - Vercel의 Framework Preset 및 Build 설정을 새 프로젝트 구조에 맞게 확인합니다.
   - `GEMINI_API_KEY` 환경 변수가 유지되어 있는지 확인합니다.
   - 배포 후 실제 URL에서 화면 이동과 `/api/recommend` AI 기능을 다시 테스트합니다.

4. **변경 후 검증**
   - 로컬 실행 → 라우팅 확인 → API 호출 확인 → 환경 변수 확인 → Vercel 배포 → 배포 URL 기능 확인 순서로 검증합니다.

   ### API 키 유출 발생 시 조치 순서

API 키가 코드, GitHub 저장소, 스크린샷 등 외부에 노출된 경우 다음 순서로 조치합니다.

1. 유출된 API 키의 사용을 즉시 중지합니다.
2. Google AI Studio에서 기존 API 키를 폐기하고 새 API 키를 발급합니다.
3. Vercel의 **Settings → Environment Variables**에서 `GEMINI_API_KEY`를 새 키로 교체합니다.
4. 코드에 API 키가 직접 입력되어 있지 않은지 확인하고 `.env`가 `.gitignore`에 포함되어 있는지 확인합니다.
5. GitHub의 커밋 이력에 실제 API 키가 노출되었는지 확인합니다.
6. 키가 커밋 이력에 포함된 경우 기존 키를 폐기한 상태에서 필요한 이력 정리를 진행합니다.
7. 새 키로 재배포한 후 배포 URL에서 AI 기능이 정상적으로 동작하는지 확인합니다.
</content>
</invoke>